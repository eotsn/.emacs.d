...
Use property lists instead of hash tables for deserialization to
increase [[https://emacs-lsp.github.io/lsp-mode/page/performance/#use-plists-for-deserialization][performance]] when communicating with language servers.
...
Make sure we download and install [[https://github.com/progfolio/elpaca][Elpaca]] if it's not available.
...
Get the latest version of =transient= from VCS instead of relying on
transient dependencies to avoid issues with loading =magit=.
